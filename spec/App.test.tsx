...
describe('App', () => {
  ...
  it('Validate calc safe per month', () => {
    ...
    const monthsToGo = wrapper.find('.saving-goal__info>span').get(0).props.children[0]
    const goalAmount = wrapper.find('.saving-goal__info>span').get(1).props.children[1]
    const currentSafePerMonth = wrapper.find('.resume__value').props().children.toString().split('$,')[1]
  ...
  it('Input goal should accept only numbers', () => {
    ...
    wrapper.find('.form__input').simulate('change', {target: { value: '-200,000.00'}})
    const mustBePositive = wrapper.find('.form__input').props().value
    const mustBePositiveFormated = Number(mustBePositive.toString().replace(/,/gm, ''))

    //Set a text
    wrapper.find('.form__input').simulate('change', {target: { value: 'aasd'}})
